...
public:
    ...
    size_t getMemoryUsage() const {
        size_t size_vector_struct = sizeof(database);
        size_t size_elements_reserved = database.capacity() * sizeof(DataArsip);
        size_t size_dynamic_strings = 0;
        for (const auto &d : database) {
            size_dynamic_strings += d.id_dokumen.capacity();
            size_dynamic_strings += d.nama_file.capacity();
            size_dynamic_strings += d.tanggal_unggah.capacity();
            size_dynamic_strings += d.sumber_data.capacity();
            size_dynamic_strings += d.konten.capacity();
        }
    ...
    if (found) {
        ...
        if (opsi == 1) {
            ...
            if (!IDDup.empty() && IDDup != id) {
                ...
                cout << "\n>> Runtime cekKonflik/duplikat : " << durDup << " microseconds\n";
                cout << ">> Runtime update Vector : " << durUpdate << " microseconds\n";
            ...
            if (konfirmasi == "y" || konfirmasi == "Y") {
                ...
                cout << "\n>> Runtime delete Vector : " << durDelete << " microseconds\n";
                cout << ">> Space Complexity (Delta): " << (int)memAfter - (int)memBefore << " bytes (Before: " << memBefore << ", After: " << memAfter << " bytes)\n";
            } else {
